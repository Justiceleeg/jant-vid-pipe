...
export function SceneCardNew({
  ...
  return (
    <div className="w-full h-full bg-card border border-border flex flex-col">
      {/* Error alert */}
      {hasError && (
        <div className="flex-shrink-0 bg-destructive/10 border-b border-destructive px-4 py-2">
          <div className="flex items-start gap-2">
            <svg className="w-4 h-4 text-destructive mt-0.5" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div className="flex-1 min-h-0 p-4">
        ...
        {scene.state === 'text' && (
          <div className="h-full flex flex-col">
            ...
            <div className="flex-1 min-h-0 flex flex-col">
              <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-2">Scene Description</h3>
              {isEditing ? (
                <div className="flex-1 flex flex-col">
                  <textarea
                    value={editedText}
                    onChange={(e) => setEditedText(e.target.value)}
                    className="flex-1 min-h-0 p-4 border-2 border-primary rounded-lg bg-background text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary"
                    disabled={isLoading}
                  />
                ...
                <p className="text-sm leading-relaxed flex-1 overflow-y-auto">{scene.text}</p>
              )}
            </div>

            {/* Actions */}
            <div className="flex items-center justify-between gap-4 pt-3 border-t border-border flex-shrink-0">
        ...
        {scene.state === 'image' && (
          <div className="h-full flex gap-4">
            ...
            <div className="flex-1 min-w-0 flex items-center justify-center">
              <div className="relative w-full h-full max-w-md mx-auto aspect-[9/16] rounded-lg overflow-hidden bg-muted border border-border">
                {isGeneratingImage ? (
            ...
            <div className="w-80 flex-shrink-0 flex flex-col gap-3">
                ...
                {isEditing ? (
                  <div className="space-y-2">
                    <textarea
                      value={editedText}
                      onChange={(e) => setEditedText(e.target.value)}
                      className="w-full min-h-[100px] p-3 border-2 border-primary rounded-lg bg-background text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary"
                      disabled={isLoading}
                    />
                    <div className="flex gap-2">
                      <Button size="sm" onClick={handleSaveText} disabled={isLoading}>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-xs leading-relaxed flex-1">{scene.text}</p>
                      <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={handleEditTextClick} disabled={isLoading} title="Edit text">
              ...
              <div className="flex-shrink-0 space-y-2 pt-2 border-t border-border">
              ...
              {/* Actions */}
              <div className="flex-shrink-0 flex flex-col gap-2 pt-2 border-t border-border">
        ...
        {scene.state === 'video' && (
          <div className="h-full flex gap-4">
            ...
            <div className="flex-1 min-w-0 flex items-center justify-center">
              <div className="relative w-full h-full max-w-md mx-auto aspect-[9/16] rounded-lg overflow-hidden bg-black border border-border">
                {isGeneratingVideo ? (
            ...
            <div className="w-80 flex-shrink-0 flex flex-col gap-3">
              ...
              <div className="flex-shrink-0 flex flex-col gap-2 pt-2 border-t border-border">
                ...
                <Button size="sm" variant="outline" onClick={onRegenerateVideo} disabled={isLoading || isGeneratingVideo} className="w-full">
                  <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                  </svg>
                  Regenerate Video
