  ...
  const generateMoodsFromBrief = useCallback(
    async (brief: MoodGenerationRequest, projectId?: string) => {
      ...
      try {
        ...
        const totalImages = response.moods.reduce((sum, mood) => sum + mood.images.length, 0);
        const successfulImages = response.moods.reduce(
          (sum, mood) => sum + mood.images.filter(img => img.success).length,
          0
        );

        if (successfulImages < totalImages) {
          console.warn(`${totalImages - successfulImages} images failed to generate`);
        }
      } catch (err) {
        const errorMessage =
