...
- **projectStore**: Syncs appStore → backend (missing some fields)
...
```
Deploy Firebase rules/functions → Start services → Smoke test full flow
```

### Phase 3: Clean Up (30 min)
```
Delete storyboard system → Move OpenAI logic to project service
```

### Phase 4: Polish (30 min)
```
Fix audio storage → Verify all assets → Firebase
```
